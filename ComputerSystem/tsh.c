...
void eval(char *cmdline)
{
    ...
    if(!is_builtin_cmd(argv)){
        if((pid = fork()) == 0){
            ...
            if(setpgid(0,0)<0){
                unix_error("eval: setgid is wrong!!!\n");
            }
        ...
        if(!bg){
            ...
            printf("[%d](%d)%s\n",get_jid_from_pid(pid),pid,cmdline);
        }
    }
    return;
}
...
void do_bgfg(char **argv)
{
    ...
    if(id==NULL){
        printf("%s command job argument\n",argv[0]);
        return;
    }

    if(id[0]=='%'){
        jid = trunce(id);
        if(!(StpJob=getjobid(jobs,jid))){
            printf("%s: No such job\n",id);
            return;
        }
    }else if(isdigit(id[0])){
        pid = trunce(id);
        if(!(StpJob=getprocessid(jobs,pid))){
    ...
    if(!strcmp(argv[0],"bg")){
        StpJob->state=BG;
        printf("[%d] (%d) %s", StpJob->jid,StpJob->cmdline);
...
void sigchld_handler(int sig)
{
    ...
    while((pid = waitpid(-1,&status,WUNTRACED | WNOHANG))>0){
        //printf("Handing child process oh yeah%d\n",(int)pid);
...
void sigtstp_handler(int sig)
{
    ...
    if(pid!=0){
        if(sig == 20){
            printf("job [%d] (%d) stopped by signal %d",jid,pid,sig);
            getprocessid(jobs,pid)->state=ST;
            kill(-pid,SIGTSTP);
        }else if(pid==-sig){
            printf("job [%d] (%d) stopped by signa; %d\n",jid,pid,20);
            getprocessid(jobs,pid)->state=ST;
        }
    }
    return;
